...
class CNN:
    ...
    def crearCapaAplanada(self,capaNoAplanada):
    ...
    def crearCapaTotalmenteConectada(self,tensorDeEntrada,num_inputs,num_outputs,use_relu=True):
        #vamos a definir los pesos y sesgos entrenables
        pesos = self._crearPesos(shape=[num_inputs, num_outputs])
        sesgos = self._crearSesgos(num_outputs)
    ...
    # crear pesos
    def _crearPesos(self,shape):
        global CNN
        return tf.Variable(tf.truncated_normal(shape, stddev=0.05))

    #crear sesgos
    def _crearSesgos(self,size):
        global CNN
        return tf.Variable(tf.constant(0.05, shape=[size]))

    #funcion para crear un capa convolucional.
    def crearCapaConvolucional(self,tensorDeEntrada, numeroDeCanales, tamanoDelFiltro,numeroDeFiltros):
        ## We shall define the weights that will be trained using create_weights function.
        ## Debemos definir los pesos que seran entrenables usango la funcion crearPesos
        pesos = self._crearPesos(shape=[tamanoDelFiltro, tamanoDelFiltro, numeroDeCanales, numeroDeFiltros])

        ## We create biases using the create_biases function. These are also trained.
        ## creamos los sesgos usanto la funcion crearSesgos. Estos son tambien entrenados
        sesgos = self._crearSesgos(size=numeroDeFiltros)
